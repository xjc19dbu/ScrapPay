...
<body>
    ...
    <?php
    require('db.php');
    $query = "SELECT * FROM `jobs` WHERE accepted='$_SESSION[username]' AND completed=0"; // Select Jobs that the user has accepted and not complete
    ...
    foreach($rows as $row){                         // Make rows from selected jobs to get data
        ...
        $tokens = $row['tokens'];
        echo "<form action='canceljob.php' class='form' method='post'> 
