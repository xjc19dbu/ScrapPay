...
<body>
    ...
    <?php
    require('db.php');
    $query = "SELECT * FROM `jobs` WHERE completed=0 AND username!='$_SESSION[username]' AND accepted=''"; // Select all jobs that are not the users and are not accepted
    ...
    foreach($rows as $row){                     // Make rows from selected jobs to get data
        ...
        $tokens = $row['tokens'];
        echo "<form action='accept.php' class='form' method='post'>
                <div class='grid-container'>
                    ...
                    <div class='grid-item'><button>Accept</button></div>
            </div>
